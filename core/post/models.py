...
class Comment(models.Model):
    ...
    author = models.ForeignKey(
        IBlogUser, on_delete=models.CASCADE, related_name="comments"
    )
    content = models.TextField()
    created_at = models.DateTimeField(auto_now_add=True, db_index=True)
...
class Reaction(models.Model):
    LIKE = "like"
    CLAP = "clap"
    REACTION_CHOICES = [(LIKE, "Like"), (CLAP, "Clap")]

    post = models.ForeignKey(
        Post, on_delete=models.CASCADE, related_name="reactions"
    )
    user = models.ForeignKey(
        IBlogUser, on_delete=models.CASCADE, related_name="reactions"
    )
    reaction_type = models.CharField(
        max_length=20, choices=REACTION_CHOICES, default=LIKE
    )
